...
'''),
            ("helpers.js", '''export const formatCurrency = (amount) => {
    return `$${amount.toFixed(2)}`;
};
'''),
            ("types.ts", '''export interface Product {
    id: string;
    name: string;
    price: number;
}
        ...
        try:
            ...
            for temp_path, original_name, content in temp_files:
                ...
                if detected_lang and SemanticChunker.is_language_supported(detected_lang):
                    ...
                    async for chunk in chunk_stream:
                        chunks.append(chunk)
                    results.append((original_name, detected_lang or "unknown", "text", len(chunks)))
            ...
            # Markdown likely uses text chunking
            md_results = [r for r in results if r[0] == "README.md"]
            assert md_results[0][2] == "text"
        finally:
            # Clean up temp files
