...
""")
    ...
    async def test_basic_async_iteration(self, test_project_dir):
        ...
        for chunk in chunks:
            ...
            assert chunk.chunk_type in [ChunkType.SEMANTIC, ChunkType.TEXT]
        ...
        has_semantic = any(chunk.chunk_type == ChunkType.SEMANTIC for chunk in chunks)
        has_text = any(chunk.chunk_type == ChunkType.TEXT for chunk in chunks)
    ...
    async def test_walker_semantic_vs_text_chunks(self, test_project_dir):
        ...
        assert len(text_chunks) > 0, "Should have text chunks from markdown files"
    ...
    async def test_walker_preserves_file_paths(self, test_project_dir):
        ...
        expected_extensions = {'.rs', '.py', '.md', '.js'}
