...
def analyze_by_language(raw_df):
    ...
    if not parser_df.empty:
        print("\nParser Performance:")
        parser_stats = (
            parser_df.groupby("language")["duration_ms"]
            .agg(
                [
                    "count",
                    "mean",
    ...
    if not tokenizer_df.empty:
        ...
        tokenizer_stats = (
            tokenizer_df.groupby("language")["duration_ms"]
            .agg(
                [
                    "count",
                    "mean",
...
def analyze_by_file_size(raw_df):
    ...
    if not parser_df.empty:
        ...
        bucket_stats = (
            parser_df.groupby("bucket")["duration_ms"]
            .agg(
                [
                    "count",
                    "mean",
        ...
        bucket_stats = bucket_stats.reindex(
            [b for b in bucket_order if b in bucket_stats.index]
        )
...
def plot_performance(raw_df, summary_df):
    ...
    if "operation" in raw_df.columns:
        ax = axes[1, 1]
        operation_stats = (
            raw_df.groupby(["language", "operation"])["duration_ms"].mean().unstack()
        )
    ...
    if not parser_df.empty:
        ...
        pivot = parser_df.pivot_table(
            values="duration_ms", index="language", columns="bucket", aggfunc="mean"
        )
    ...
    if not parser_df.empty:
        ...
        if "operation" in raw_df.columns:
            ...
            if "parser" in op_pivot.columns and "tokenizer" in op_pivot.columns:
                ...
                ratio_by_lang = (
                    op_pivot.groupby(level="language")["ratio"]
                    .agg(["mean", "std", "max"])
                    .round(1)
                )
                print(ratio_by_lang)
