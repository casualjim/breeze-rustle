...
def analyze_by_language(raw_df):
    ...
    if not parser_df.is_empty():
        print("\nParser Performance:")
        parser_stats = parser_df.group_by("language").agg([
    ...
    if not tokenizer_df.is_empty():
        ...
        tokenizer_stats = tokenizer_df.group_by("language").agg([
            pl.count("duration_ms").alias("count"),
            pl.mean("duration_ms").alias("mean"),
            pl.std("duration_ms").alias("std"),
            pl.min("duration_ms").alias("min"),
...
def analyze_by_file_size(raw_df):
    ...
    if not parser_df.is_empty():
        ...
        throughput_by_bucket = parser_with_throughput.group_by("bucket").agg([
            pl.mean("throughput").alias("avg_throughput")
        ]).with_columns([
            pl.col("bucket").cast(pl.Enum(bucket_order))
        ]).sort("bucket")
...
def analyze_processing_log(log_df):
    ...
    # Group by language
    language_stats = log_df.group_by("language").agg([
        pl.count("duration_ms").alias("count"),
...
def plot_performance(raw_df, summary_df, log_df):
    ...
    if not parser_df.is_empty():
        ...
        for lang in languages:
            ...
            ax.scatter(lang_data['file_size'], lang_data['duration_ms'], 
    ...
    if log_df is not None and not log_df.is_empty():
        ...
        for lang in log_pd['language'].unique():
            ...
            ax.plot(lang_data['timestamp'], lang_data['cumulative'], 
                   label=lang, marker='o', markersize=2)
    ...
    if 'operation' in raw_df.columns:
        ...
        operation_stats = raw_df.group_by(["language", "operation"]).agg([
            pl.mean("duration_ms").alias("avg_duration")
        ]).pivot(
            values="avg_duration",
            index="language",
            columns="operation"
    ...
    if not parser_df.is_empty():
        ...
        pivot_df = parser_df.group_by(["language", "bucket"]).agg([
            pl.mean("duration_ms").alias("avg_duration")
        ]).pivot(
            values="avg_duration",
            index="language",
        ...
        # Create heatmap
        sns.heatmap(pivot_pd, annot=True, fmt='.1f', cmap='YlOrRd', 
...
def identify_bottlenecks(raw_df, log_df):
    ...
    if not parser_df.is_empty():
        ...
        outlier_summary = outliers.group_by(["language", "bucket"]).agg([
            pl.count("duration_ms").alias("count")
        ...
        if 'operation' in raw_df.columns:
            ...
            parser_agg = raw_df.filter(pl.col("operation") == "parser").group_by(["language", "file_size"]).agg([
                pl.mean("duration_ms").alias("parser_ms")
            ])
            
            tokenizer_agg = raw_df.filter(pl.col("operation") == "tokenizer").group_by(["language", "file_size"]).agg([
                pl.mean("duration_ms").alias("tokenizer_ms")
            ])
            
            joined = parser_agg.join(tokenizer_agg, on=["language", "file_size"], how="inner")
            joined = joined.with_columns([
                (pl.col("parser_ms") / pl.col("tokenizer_ms")).alias("ratio")
            ])
            
            # Average ratio by language
            ratio_by_lang = joined.group_by("language").agg([
    ...
    if log_df is not None and not log_df.is_empty():
        ...
        p10_throughput = log_with_throughput.select(pl.quantile("throughput", 0.1)).item()
        ...
        for row in slow_sorted.rows():
            ...
            duration = row[4]
            throughput = row[6]
            print(f"  {file_path} ({language}, {size/1024:.1f}KB): {throughput:.1f} bytes/ms")
...
def main():
    ...
    try:
        # Need pandas for some matplotlib compatibility
        import pandas as pd
