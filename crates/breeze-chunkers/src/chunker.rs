use crate::{languages::*, metadata_extractor::extract_metadata_from_tree, types::*, Tokenizer};
...
mod tests {
    use super::*;
    use crate::{chunker::InnerChunker, Tokenizer, types::Chunk};
...
"#;
    ...
    #[tokio::test]
    #[ignore = "JavaScript support not yet implemented in breeze-grammars"]
...
"#;
    ...
    #[tokio::test]
    #[ignore = "Rust support not yet implemented in breeze-grammars"]
...
"#;
        ...
        let has_struct_or_impl = chunks.iter().any(|c| match c {
            Chunk::Semantic(sc) => sc.metadata.definitions.iter().any(|d| d == "Cache" || d == "new" || d == "get" || d == "insert"),
            _ => false,
        });
    ...
    #[tokio::test]
    #[ignore = "TypeScript support not yet implemented in breeze-grammars"]
...
"#;
        ...
        let has_nested_class = chunks.iter().any(|c| match c {
            Chunk::Semantic(sc) => {
                ...
                sc.metadata.node_name.as_ref().map_or(false, |n| n == "NestedClass")
            },
            _ => false,
        });
...
"#;
            ...
            match chunk {
                Chunk::Semantic(sc) => {
                    ...
                    assert_eq!(open_parens, close_parens, 
                        "Chunk should have balanced parentheses: {}", sc.text);
                }
                _ => panic!("Expected semantic chunk"),
            }
        }
    ...
    async fn test_line_numbers_are_1_based() {
        ...
        for chunk in chunks {
            match chunk {
                Chunk::Semantic(sc) => {
                    assert!(sc.start_line >= 1, "Start line should be 1-based, got {}", sc.start_line);
                    assert!(sc.end_line >= 1, "End line should be 1-based, got {}", sc.end_line);
                    assert!(sc.end_line >= sc.start_line, "End line should be >= start line");
                }
                _ => panic!("Expected semantic chunk"),
            }
        }
