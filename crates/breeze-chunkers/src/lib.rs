...
pub use crate::walker::{walk_project, WalkOptions};
...
impl Default for Chunker {
    ...
    fn default() -> Self {
        Self::new(ChunkerConfig::default())
            .expect("Default chunker creation should never fail")
    }
}
    ...
    pub async fn chunk_file(
        ...
        if let Ok(Some(detection)) = hyperpolyglot::detect(path) {
            ...
            if is_language_supported(language) {
                return Ok(self.chunk_code(content, language.to_string(), Some(file_path)).boxed());
            }
        }
