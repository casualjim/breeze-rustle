...
impl PerformanceTracker {
  pub fn new() -> Self {
    ...
    if needs_header {
      writeln!(
        log_file,
        "timestamp_ms,file_path,language,file_size,duration_ms,operation"
      )
      .ok();
    }
  ...
  pub fn record(&self, language: String, file_size: u64, duration: Duration, operation: &str) {
    ...
    if let Ok(mut file) = self.get_language_file(&language) {
      writeln!(
        file,
        "{},{},{},{:.3},{}",
        timestamp, file_size, bucket_label, duration_ms, operation
      )
      .ok();
      file.flush().ok();
    }
  ...
  pub fn record_file_processing(
    ...
    if let Ok(mut log_file) = self.log_file.lock() {
      writeln!(
        log_file,
        "{},{},{},{},{:.3},{}",
        timestamp, file_path, language, file_size, duration_ms, operation
      )
      .ok();
      log_file.flush().ok();
    }
  ...
  fn write_summary(
      ...
      .open(&summary_path)
    {
      if needs_header {
        writeln!(
          file,
          "timestamp_ms,language,bucket,count,avg_ms,min_ms,max_ms"
        )
        .ok();
      }
      ...
      let mut sorted_languages: Vec<_> = languages
        .iter()
