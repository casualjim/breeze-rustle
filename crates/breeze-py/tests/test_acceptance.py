import pytest
from breeze import SemanticChunker, SemanticChunk, ChunkMetadata, TokenizerType, ChunkType
...
class TestBasicFunctionality:
    def test_supported_languages(self):
        ...
        core_languages = ["python", "javascript", "java", "cpp", "c", "csharp", "typescript", "sql", "php", "go"]
        for lang in core_languages:
            assert lang in languages, f"{lang} should be supported"
        
        # Test popular languages (11-20)
        popular_languages = ["rust", "swift", "kotlin", "ruby", "r", "bash", "scala", "dart", "powershell"]
        for lang in popular_languages:
            assert lang in languages, f"{lang} should be supported"
        
        # Test specialized languages (21-30)
        specialized_languages = ["objc", "perl", "fortran", "matlab", "lua", "haskell", "asm", "pascal", "ada"]
        for lang in specialized_languages:
            assert lang in languages, f"{lang} should be supported"
        
        # Test emerging & niche (31-41)
        emerging_languages = ["elixir", "erlang", "commonlisp", "julia", "clojure", "solidity", "apex", "groovy", "ocaml", "scheme", "zig"]
...
enum Result<T, E> {
    ...
    export namespace HR {
        ...
        assert any(c.metadata.node_type == "internal_module" or "namespace" in str(c.metadata.scope_path) for c in chunks)
...
class MyClass {
  ...
  defp validate_email(email) do
    String.contains?(email, "@")
        ...
        assert any("defmodule" in c.metadata.node_type or c.metadata.node_type == "call" for c in chunks)
