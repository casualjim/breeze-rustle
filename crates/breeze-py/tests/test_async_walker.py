...
""")
    ...
    async def test_basic_async_iteration(self, test_project_dir):
        ...
        for chunk in chunks:
            ...
            assert chunk.chunk.chunk_type in [
                ChunkType.SEMANTIC,
                ChunkType.TEXT,
                ChunkType.EOF,
            ]
        ...
        has_semantic = any(
            chunk.chunk.chunk_type == ChunkType.SEMANTIC for chunk in chunks
        )
            ...
            for chunk in chunks:
                ...
                if chunk.chunk.chunk_type != ChunkType.EOF:
                    assert chunk.chunk.metadata.language, (
                        "Non-EOF chunks should have language"
                    )
    ...
    async def test_walker_respects_max_parallel(self, test_project_dir):
        ...
        for max_parallel in [1, 2, 8]:
            ...
            walker = await chunker.walk_project(
                test_project_dir, max_parallel=max_parallel
            )
    ...
    async def test_walker_small_chunk_size(self, test_project_dir):
        ...
        assert len(oversized_chunks) / len(chunks) < 0.5, (
    ...
    async def test_eof_chunks_contain_content_and_hash(self, test_project_dir):
        ...
        async for chunk in walker:
            if chunk.chunk.chunk_type == ChunkType.EOF:
                ...
                assert hasattr(chunk.chunk, "content"), (
                    "EOF chunk should have content attribute"
                )
                assert hasattr(chunk.chunk, "content_hash"), (
                    "EOF chunk should have content_hash attribute"
                )
                assert chunk.chunk.content is not None, (
                    "EOF chunk content should not be None"
                )
                assert chunk.chunk.content_hash is not None, (
                    "EOF chunk content_hash should not be None"
                )
                assert isinstance(chunk.chunk.content, str), (
                    "EOF chunk content should be a string"
                )
                assert isinstance(chunk.chunk.content_hash, bytes), (
                    "EOF chunk content_hash should be bytes"
                )
                assert len(chunk.chunk.content_hash) == 32, (
                ...
                assert chunk.chunk.content is None, (
                    "Non-EOF chunks should have None content"
                )
                assert chunk.chunk.content_hash is None, (
                    "Non-EOF chunks should have None content_hash"
                )

        # Should have EOF chunks for each file
        assert len(eof_chunks) > 0, "Should have EOF chunks"
        assert len(eof_chunks) == len(file_contents), (
            "Should have one EOF chunk per file"
        )
        ...
        for file_path, content in file_contents.items():
            with open(file_path, "r") as f:
                ...
                assert content == actual_content, (
                    f"EOF chunk content should match file content for {file_path}"
                )
        ...
        for file_path in file_contents:
            ...
            if file_chunks:
                ...
                for chunk in file_chunks:
                    assert chunk.chunk.text in full_content, (
                        "Chunk text should be found in full content"
                    )
