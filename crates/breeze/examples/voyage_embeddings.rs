use anyhow::Result;
use breeze::aiproviders::voyage::{self, Config, Tier, EmbeddingModel, EmbeddingRequest, VoyageClient};
use std::env;
...
async fn main() -> Result<()> {
    ...
    let estimated_tokens: u32 = texts.iter()
        .map(|t| (t.len() / 4) as u32)
        .sum();

    // Get embeddings
    println!("Embedding {} texts (estimated {} tokens)...", texts.len(), estimated_tokens);
