...
use breeze_chunkers::{Chunk, WalkOptions, walk_project, Tokenizer};
...
async fn run_embedding() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Benchmarking embeddings for directory: {}", directory.display());
    ...
    if total_chunks > 0 {
        ...
        println!("Average chunks per file: {:.1}", total_chunks as f64 / total_files as f64);
    }

    if total_tokens > 0 {
        println!("Tokens per second: {:.0}", total_tokens as f64 / elapsed.as_secs_f64());
    }
...
fn embedding_worker_sync(
    ...
    info!("Worker {} completed. Processed {} batches", worker_id, batches_processed);
}
...
fn process_chunk_batch_sync(
    ...
    *batches_processed += 1;
    debug!("Worker {} processing batch {} with {} chunks",
        worker_id, batches_processed, chunks.len());
    ...
    match rt.block_on(embedder.embed(&texts, None, None)) {
        Ok(embeddings) => {
            stats.embeddings.fetch_add(embeddings.len(), Ordering::Relaxed);

            // Count approximate tokens
            for chunk in chunks {
                stats.tokens.fetch_add(chunk.text.len() / 4, Ordering::Relaxed);
            }
            ...
            debug!("Worker {} successfully embedded {} chunks", worker_id, embeddings.len());
        }
        Err(e) => {
