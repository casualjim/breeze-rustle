...
use crate::pipeline::{
    PathWalker, Batcher, Embedder, Aggregator, RecordBatchConverter, Sink
};
...
impl<'a> Indexer<'a> {
    ...
    pub async fn index(
        ...
        use breeze_chunkers::{WalkOptions, Tokenizer};
        ...
        let batcher = PassthroughBatcher::new(
            NonZeroUsize::new(self.config.max_parallel_files).unwrap()
        );
