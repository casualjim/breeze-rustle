    ...
    let encodings = text_batch
      ...
      .flat_map(|mini_text_batch| -> Result<Vec<Vec<f32>>, E> {
        ...
        let outputs = model_guard.run(ort::inputs![
            "input_ids" => ort::value::TensorRef::from_array_view(&token_ids)?,
            "input_mask" => ort::value::TensorRef::from_array_view(&attention_mask)?,
            "segment_ids" => ort::value::TensorRef::from_array_view(&token_type_ids)?
        ])?;
        let embeddings: ndarray::ArrayViewD<f32> = outputs["output"].try_extract_array()?;
        let embeddings = embeddings.into_dimensionality::<Ix3>()?.to_owned();
