...
use crate::aiproviders::voyage::{
    Config as VoyageClientConfig, EmbeddingModel, EmbeddingRequest,
    Tier, new_client, VoyageClient
};
use crate::config::VoyageConfig;
use super::{EmbeddingProvider, batching::{BatchingStrategy, TokenAwareBatchingStrategy}};
...
impl VoyageEmbeddingProvider {
    pub async fn new(config: &VoyageConfig) -> Result<Self, Box<dyn std::error::Error>> {
        let client_config = VoyageClientConfig::new(
...
impl EmbeddingProvider for VoyageEmbeddingProvider {
    async fn embed(&self, inputs: &[super::EmbeddingInput<'_>]) -> Result<Vec<Vec<f32>>, Box<dyn std::error::Error + Send + Sync>> {
        // Prepare request
        ...
        let estimated_tokens: u32 = inputs.iter()
            .map(|input| {
