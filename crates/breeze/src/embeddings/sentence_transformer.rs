...
use crate::embeddings::{EmbeddingError, models::{SentenceTransformerModel, ModelType, PoolingMethod}};
...
impl SentenceTransformerEmbedder {
    ...
    async fn embed_batch(&self, texts: &[String]) -> Result<Vec<Vec<f32>>, EmbeddingError> {
        ...
        let input_ids_tensor = Tensor::new(flat_ids.as_slice(), &self.device)
            ...
            .map_err(|e| EmbeddingError::InferenceError(format!("Failed to reshape input tensor: {}", e)))?;
        ...
        let attention_mask_tensor = Tensor::new(flat_mask.as_slice(), &self.device)
            ...
            .reshape(&[texts.len(), max_len])
            .map_err(|e| EmbeddingError::InferenceError(format!("Failed to reshape mask tensor: {}", e)))?;
        
        // Run model inference
        let model = self.model.lock().await;
        let embeddings = model.forward(&input_ids_tensor, &attention_mask_tensor)
...
impl Embedder for SentenceTransformerEmbedder {
    fn embed(&self, batches: BoxStream<TextBatch>) -> BoxStream<EmbeddingBatch> {
        ...
        let stream = batches.then(move |batch| {
