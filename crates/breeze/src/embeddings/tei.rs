...
use crate::pipeline::{EmbeddedChunk, ProjectFileWithEmbeddings, Embedder, BoxStream};
...
impl TEIEmbedder {
    ...
    pub async fn new(
        ...
        let embedding_dim = std::fs::File::open(&config_path)
            ...
            .and_then(|config| {
                config["hidden_size"]
                    ...
                    .ok_or_else(|| EmbeddingError::ModelLoadError("Config missing 'hidden_size' field".to_string()))
            })?;
        ...
        info!("TEI model loaded successfully, max batch size: {}, embedding dim: {}", max_batch_size, embedding_dim);
    ...
    async fn embed_chunk_batch(&self, chunks: Vec<Chunk>) -> Result<Vec<EmbeddedChunk>, EmbeddingError> {
        if chunks.is_empty() {
            return Ok(Vec::new());
        }
            ...
            if let Some(embedding) = embeddings.remove(&i) {
                let embedding_vec = match embedding {
                    ...
                    Embedding::All(vecs) => {
                        ...
                        if vecs.is_empty() {
                            return Err(EmbeddingError::InferenceError("Empty embedding vector".to_string()));
                        }
                        let dim = vecs[0].len();
                ...
                return Err(EmbeddingError::InferenceError(format!("Missing embedding for chunk {}", i)));
            }
        }
    ...
    fn prepare_batch_from_owned(&self, chunks: Vec<Chunk>) -> Result<(Batch, Vec<Chunk>), EmbeddingError> {
        ...
        for (idx, chunk) in chunks.into_iter().enumerate() {
            ...
            let chunk_tokens = semantic_chunk.tokens.as_ref()
                .ok_or_else(|| EmbeddingError::InferenceError(
                    "TEI embedder requires pre-tokenized chunks. Use HuggingFace tokenizer in chunker.".to_string()
                ))?;
...
impl Embedder for TEIEmbedder {
    fn embed(&self, files: BoxStream<ProjectFile>) -> BoxStream<ProjectFileWithEmbeddings> {
        ...
        let stream = files.map(move |project_file| {
            let embedder = embedder.clone();
            let file_path = project_file.file_path.clone();
            let metadata = project_file.metadata.clone();
            
            // Create a stream that batches chunks and embeds them
            let embedded_chunks = project_file.chunks
                .chunks(batch_size)
                .then(move |chunk_batch| {
                    let embedder = embedder.clone();
                    let file_path = file_path.clone();
                    
                    async move {
                        // Collect the batch, filtering out errors
                        ...
                        for chunk_result in chunk_batch {
                            match chunk_result {
                                Ok(chunk) => {
                                    // Check if chunk has pre-tokenized data
                                    let semantic_chunk = match &chunk {
                                        Chunk::Semantic(sc) | Chunk::Text(sc) => sc,
                                    };
                                    
                                    if semantic_chunk.tokens.is_none() {
                                        errors.push((valid_chunks.len(), ChunkError::ParseError(
                                            "TEI embedder requires pre-tokenized chunks. Use HuggingFace tokenizer in chunker.".to_string()
                                        )));
                                    } else {
                                ...
                                Err(e) => errors.push((valid_chunks.len(), e)),
                            }
                        }
                        
                        // Process valid chunks if any
                        let result_stream: Box<dyn futures_util::Stream<Item = Result<EmbeddedChunk, ChunkError>> + Send + Unpin> = if !valid_chunks.is_empty() {
                            match embedder.embed_chunk_batch(valid_chunks).await {
                                Ok(embedded_chunks) => {
                                    // Return embedded chunks as a stream
                                    Box::new(futures_util::stream::iter(embedded_chunks.into_iter().map(Ok)))
                                }
                                Err(e) => {
                                    error!(
                                        error = %e,
                                    ...
                                    // Return error for all chunks in batch
                                    let err = ChunkError::ParseError(format!("Embedding generation failed: {}", e));
                                    Box::new(futures_util::stream::iter(vec![Err(err)]))
                                }
                            }
                        } else {
                            // All chunks had errors
                            Box::new(futures_util::stream::iter(errors.into_iter().map(|(_, e)| Err(e))))
                        };
                        result_stream
                    }
                })
            ...
            ProjectFileWithEmbeddings {
                ...
                metadata,
                embedded_chunks: Box::pin(embedded_chunks),
            }
        });
