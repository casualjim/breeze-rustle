...
use arrow::array::{StringArray, FixedSizeBinaryArray, Float32Array, FixedSizeListArray, UInt64Array, TimestampNanosecondArray, NullArray};
...
impl<T> MockRecordBatchConverter<T> {
    ...
    pub fn with_failure(batch_size: NonZeroUsize, schema: Arc<arrow::datatypes::Schema>, fail_after: usize) -> Self {
        Self {
            batch_size,
            schema,
    ...
    fn convert(&self, items: BoxStream<T>) -> Pin<Box<dyn RecordBatchStream + Send>> {
        ...
        let batch_stream = items
            ...
            .map(move |items| {
                ...
                let arrays: Vec<Arc<dyn arrow::array::Array>> = schema_clone.fields()
                    .iter()
                    .map(|field| {
                        match field.name().as_str() {
                            "id" => Arc::new(arrow::array::StringArray::from(
                                vec!["mock-id"; num_rows]
                            )) as Arc<dyn arrow::array::Array>,
                            "file_path" => Arc::new(arrow::array::StringArray::from(
                                vec!["mock-path"; num_rows]
                            )) as Arc<dyn arrow::array::Array>,
                            "content" => Arc::new(arrow::array::StringArray::from(
                                vec!["mock-content"; num_rows]
                            )) as Arc<dyn arrow::array::Array>,
                            "content_hash" => {
                                Arc::new(arrow::array::FixedSizeBinaryArray::from(
                                    vec![[0u8; 32].as_ref(); num_rows]
                                )) as Arc<dyn arrow::array::Array>
                            },
                            "content_embedding" => {
                                // Get the embedding dimension from the field type
                                let dim = if let arrow::datatypes::DataType::FixedSizeList(_, size) = field.data_type() {
                                    *size as usize
                                } else {
                                    384 // default
                                };
                            ...
                            "file_size" => Arc::new(arrow::array::UInt64Array::from(
                                vec![100u64; num_rows]
                            )) as Arc<dyn arrow::array::Array>,
        ...
        Box::pin(SimpleRecordBatchStream::new(batch_stream, self.schema.clone()))
    }
}
