...
pub struct RateLimitMiddleware {
    ...
    request_limiter: Arc<RateLimiter<NotKeyed, InMemoryState, QuantaClock, NoOpMiddleware<QuantaInstant>>>,
    /// Rate limiter for tokens per minute
    token_limiter: Arc<RateLimiter<NotKeyed, InMemoryState, QuantaClock, NoOpMiddleware<QuantaInstant>>>,
}
...
impl RateLimitMiddleware {
    ...
    async fn acquire_tokens(&self, token_count: u32) -> Result<()> {
        ...
        debug!("Waiting for request slot...");
        self.request_limiter.until_ready_with_jitter(governor::Jitter::up_to(Duration::from_millis(100))).await;
        debug!("Request slot acquired");
        ...
        loop {
            match self.token_limiter.check_n(tokens_needed) {
                ...
                Err(_) => {
                    ...
                    self.token_limiter.until_ready_with_jitter(governor::Jitter::up_to(Duration::from_millis(100))).await;
