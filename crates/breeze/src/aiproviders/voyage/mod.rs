...
pub use types::{Tier, EmbeddingResult, FileContent};
...
#[async_trait]
pub trait VoyageClient {
    async fn embed(&self, request: EmbeddingRequest, estimated_tokens: u32) -> Result<EmbeddingResponse, Error>;
...
mod tests {
    ...
    fn test_voyage_config() {
        let config = Config::new("test_key".to_string(), Tier::Free, EmbeddingModel::VoyageCode3)
