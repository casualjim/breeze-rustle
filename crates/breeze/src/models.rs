...
impl CodeDocument {
    ...
    pub fn schema(embedding_dim: usize) -> arrow::datatypes::Schema {
        ...
        let fields = vec![
            ...
            Field::new("last_modified", DataType::Timestamp(TimeUnit::Nanosecond, None), false),
            Field::new("indexed_at", DataType::Timestamp(TimeUnit::Nanosecond, None), false),
        ];
...
impl lancedb::arrow::IntoArrow for CodeDocument {
    fn into_arrow(self) -> lancedb::Result<Box<dyn arrow::array::RecordBatchReader + Send>> {
        ...
        let embedding_field = Arc::new(arrow::datatypes::Field::new("item", arrow::datatypes::DataType::Float32, true));
        let embedding_list = FixedSizeListArray::try_new(
            embedding_field,
            embedding_dim as i32,
        ...
        // Convert timestamps
        let last_modified_ns = self.last_modified.and_utc().timestamp_nanos_opt().unwrap_or(0);
