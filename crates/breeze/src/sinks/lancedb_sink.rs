...
use crate::pipeline::{Sink, BoxStream};
...
mod tests {
    ...
    use arrow::array::{StringArray, FixedSizeBinaryArray, Float32Array, FixedSizeListArray, UInt64Array, TimestampNanosecondArray};
    use lancedb::arrow::IntoArrow;
    use lancedb::query::{ExecutableQuery, QueryBase};
    
    async fn create_test_table(db_path: &str, table_name: &str, embedding_dim: usize) -> lancedb::Table {
        ...
        if table_names.contains(&table_name.to_string()) {
            ...
            let table = conn.create_table(table_name, reader).execute().await.unwrap();
    ...
    fn create_test_batch(docs: Vec<CodeDocument>) -> RecordBatch {
        ...
        let all_embeddings: Vec<f32> = docs.iter()
        ...
        // Convert timestamps
        let last_modified: Vec<i64> = docs.iter()
            .map(|d| d.last_modified.and_utc().timestamp_nanos_opt().unwrap_or(0))
            .collect();
        let indexed_at: Vec<i64> = docs.iter()
    ...
    async fn test_lancedb_sink_upsert() {
        ...
        let mut query_stream = table.query()
        ...
        let content_array = content_array.as_any().downcast_ref::<StringArray>().unwrap();
