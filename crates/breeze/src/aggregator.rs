...
impl Aggregator for FileAggregator {
  fn aggregate(&self, embeddings: BoxStream<EmbeddingBatch>) -> BoxStream<CodeDocument> {
    ...
    let collected = embeddings.fold(
      HashMap::<String, FileAccumulator>::new(),
      |mut acc, batch| async move {
        for item in batch {
    ...
    let stream = collected.into_stream().flat_map(|accumulators| {
...
mod tests {
  ...
  async fn test_file_aggregation() {
    ...
    let batch1 = vec![
      EmbeddingBatchItem {
        ...
        metadata: vec![create_test_chunk(
    ...
    let batch2 = vec![EmbeddingBatchItem {
  ...
  async fn test_single_file_single_chunk() {
    ...
    let batch = vec![EmbeddingBatchItem {
  ...
  async fn test_multiple_batches_same_file() {
    ...
    let batch1 = vec![EmbeddingBatchItem {
      embeddings: vec![1.0],
